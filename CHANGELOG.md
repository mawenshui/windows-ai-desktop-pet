...
- 待办页新增“专注陪伴”：支持 15/25/50 分钟快捷时长、5～180 分钟自定义时长、可选关联未完成待办，以及开始、暂停、继续、提前结束和到时确认。
- 新增 `focus-sessions.json` schema 1，以原子写入和 revision 保存最多一个活动会话；支持重启、休眠与系统时间变化恢复，并在复盘及 Markdown 中汇总次数、总时长和提前结束次数。
- 设置新增专注时点击穿透、同屏全屏时自动隐藏桌宠；托盘提供暂停/继续、结束专注和恢复桌宠交互入口。
- 内置四角色选择改为真实首帧卡片，展示中文名、选中状态、CC0 许可与本地许可入口。

### Changed

- 专注运行或暂停时停止桌宠自主漫游、随机气泡和朝向跟随；正式提醒仍可显示，到时在系统允许动画时只播放一次现有完成动作。
- 设置升级为 schema 6；专注记录进入手动/自动备份、恢复事务与卸载清理，且不进入 AI、搜索、日志、诊断或遥测。
- 桌宠点击/拖拽边界改用屏幕坐标，并以已经进入拖拽状态作为松开判定依据，避免自主漫游期间的普通单击被窗口位移误判为拖拽。

### Tests

- 新增专注状态机、恢复/时间回拨、聚合、损坏保护、维护校验、设置迁移、全屏几何策略、角色切换事务、低打扰显式保存和 WPF 合同回归。
- 性能探针按角色图像实时位置发送输入，从实际单击开始统计工具窗唤起，并以原生 16 ms 节拍采集拖拽观测值；前台或目标归属不确定时拒绝发送输入。

## [0.24.0] - 2026-09-15

### Added

- 待办页新增“清单 / 复盘”二级视图：显示本地计算的完成/待处理/已安排摘要和相关事项只读列表，不增加主页面。
- 新增独立 `journal.json` schema 1：4,000 字正文、500 ms 自动保存、单调修订保护、跨日一次性快照、历史浏览/显式编辑与逐日删除。
- 新增用户主动的单日 Markdown 导出，默认 `YYYY-MM-DD.md`，使用 UTF-8 无 BOM、LF 和原子写入；不创建默认导出目录或隐式搜索授权。
- 每日复盘进入手动备份、每日自动备份、恢复事务、回滚与卸载清理，同时保持在 AI、搜索、日志、诊断和遥测之外。

### Tests

- 新增复盘存储、损坏/并发写与 32 MiB 容量保护、日期投影、跨日冻结、删除隔离、Markdown 编码/净化/目录目标保护、备份恢复、自动备份、ViewModel 和 WPF 合同回归。
- 打包与系统 E2E 脚本兼容 Windows PowerShell 5.1，同时保持候选文件原子发布和 UTF-8 无 BOM 报告。

## [0.23.0] - 2026-09-14

### Changed

- 提醒中心增加仅匹配标题的本地查找、待处理/历史/全部三筛选计数、处理优先/最近记录两种稳定排序和上下文空态。
- 提醒记录改为原生单选列表，刷新按稳定通知 ID 恢复仍可见选择；完成、稍后和详情集中到唯一的所选提醒操作带。
- 列表增加 Enter/F2 详情、`Ctrl+Enter` 完成、`Ctrl+Shift+Enter` 稍后，并为类型、恢复来源、投递状态、目标和操作补充非颜色文字及 UI Automation 名称。
- 静默时段改为显式草稿，增加严格 `HH:mm` 实时校验、夜间/午休/关闭预设、未保存/已保存状态和独立提醒反馈区。
- 清理历史显示实际可清数量，继续保留排队/已提交记录和业务待办；查询、筛选、排序、选择及草稿状态均不持久化。

### Tests

- 新增提醒投影、筛选计数、排序选择、目标动作、历史清理、静默草稿/预设/校验和 WPF 生命周期/布局合同回归；完整套件为 338 项。

## [0.22.0] - 2026-09-14

### Changed

- 待办页增加只在本地匹配标题与备注的查找、`Ctrl+F` 聚焦、查询专用空态、六筛选计数和三种进程内排序。
- 手动清单改为原生单选列表；刷新按稳定 ID 恢复仍可见事项，当前项动作集中到统一操作带。
- 列表支持方向键/Home/End、Enter/F2 编辑、`Ctrl+Enter` 完成或恢复、Delete 可撤销删除，并按事项状态显示可用动作。
- 卡片增加已逾期、今天、未来、未安排、已完成等文字紧迫度，列表与操作带补充目标化 UI Automation 名称和动态选择摘要。
- 编辑器增加截止与提醒快捷时间、标题/备注长度计数、实时有效性提示，并在字段修正后清理过期提交错误。

### Tests

- 新增待办查找/计数/排序、选择恢复、统一操作、键盘语义、时间快捷值、长度边界、实时校验和 WPF 绑定回归。

## [0.21.0] - 2026-09-13

### Changed

- 工具窗增加 `Ctrl+N`、`Ctrl+Enter` 和 `F1`，页签展示快捷键提示，Esc 先处理待办编辑或搜索再收起。
- 搜索无结果可一键重置查询、类别和范围，并提供授权范围管理；独立显示结果数量与选中序号。
- 待办增加筛选摘要、上下文空态、全字段未保存保护和页内放弃确认。
- 手动新建、修改、完成、恢复、稍后、取消提醒和删除统一支持最近一次撤销，并在目标后续变更时拒绝覆盖；删除改为可恢复的直接操作。
- 今日安排可选择前 12 条或清除选择；紧凑操作至少 36 DIP，时间格式、状态和错误增加可访问文本与动态播报。
- 维护操作增加独立状态通道，失败只显示稳定恢复建议，不暴露原异常或完整路径。

### Tests

- 新增搜索重置/计数、待办编辑保护/统一撤销/并发拒绝、今日安排批量选择、维护状态和 WPF Esc 层级回归。

## [0.20.0] - 2026-09-13

### Added

- 内置“GitHub 官方 + 智能加速”分级更新线路：元数据在官方失败后自动切换，安装器下载按三条内置线路及官方地址逐级恢复，并在界面显示当前使用的可理解线路状态。
- Release 元数据支持 GitHub `browser_download_url`、资产大小和可选 `sha256:` digest；digest 与 `SHA256SUMS.txt` 必须一致后才允许准备安装。

### Changed

- 设置页移除 `{url}` 加速模板、代理地址和 GitHub Token 输入，只保留周期、间隔、检查、下载与智能线路状态，普通用户无需配置网络专业项。
- 更新检查固定为公开仓库的稳定 Release，所有内置线路保持匿名请求；旧加速模板被兼容读取后清空，旧更新凭据不再读取或发送。
- 收紧仓库、标签、文件名、HTTPS、用户信息、重定向主机、响应大小和临时文件边界；单条线路失败不清除已发现版本。

### Tests

- 新增内置线路顺序、失败回退、匿名请求、digest/清单一致性、安全重定向、旧设置兼容、ViewModel 状态和设置页减负回归。

## [0.19.0] - 2026-09-13

### Changed

- 修复主页搜索结果操作、快捷入口和状态行重叠；扩大标题栏及快捷管理命中区，并为失效入口补充非颜色警示。
- 增加搜索框与首条结果的上下键往返、搜索内优先 Esc、`Ctrl+L` 和 `Alt+1`～`Alt+4` 直接页导航，统一保留 AI 未保存保护。
- 将待办手动新建、筛选、编辑和清单前置；一句话 AI 与今日安排按需展开，草稿/错误自动展开，补齐新建和关闭后的焦点恢复。
- 增加设置五项目录的滚动聚焦和 AI 自动展开；缓存重建改用危险次要层级、明确后果和默认保留取消。
