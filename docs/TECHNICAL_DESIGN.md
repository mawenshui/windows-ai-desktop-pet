...
日期：2026-09-09；软件：0.16.0 候选工作区。本文以当前代码为准；[实施追踪](0.16.0－功能扩展计划.md)、[验证报告](release/0.16.0-test-report.md)分别说明范围与验收状态。
...
TodoStore.UpdateBatch 在同一存储锁内核对每项 `UpdatedAt`、验证全部候选，再一次原子写入；任一标识缺失、旧快照或规则错误使整批不写。RestoreBatch 使用确认后 `UpdatedAt` 防止撤销覆盖后续编辑。今日安排批量撤销只留在内存。
...
今日安排是独立协议。ViewModel 只从用户明确勾选的 1～12 条待处理事项构造 ID、标题、备注、截止时间和 `UpdatedAt` 快照；请求不含未勾选或已完成事项。AI 必须原样覆盖全部 ID，返回当前偏移下今天未来的 15 分钟～4 小时时间块；额外/重复/缺失 ID、无绝对偏移、跨日、过去或重叠结果直接拒绝。本地降级按截止时间排序，从下一刻钟开始分配 30 分钟块，不访问网络。两者都先形成内存草稿，逐项勾选确认后才批量写入。
