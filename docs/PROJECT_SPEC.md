...
| 属性 | 值 |
| :--- | :--- |
| 文档版本 | 1.9（2026-09-12 代码状态核对） |
| 软件基线版本 | 0.18.0 |
| 状态 | 生效；WPF/.NET 8 技术栈已落地，0.18.0 为候选工作区 |
| 适用范围 | 源码、测试、文档、构建、打包、版本、GitHub 与 AI 工具协作 |
| 需求基线 | `docs/Windows桌面宠物产品需求文档_PRD.md` V2.7 |
...
当前仓库已有 WPF/.NET 8 产品源码、CC0 RGS 桌宠、页内单选与四页工具窗、自动化测试，以及便携版和 Inno Setup 安装版。0.18.0 在现有搜索结果下增加可访问操作区，并用独立系统动作边界处理打开、资源管理器定位和剪贴板；加入快捷入口继续复用原有存储及去重规则。EXT-08 外部宠物包仍只在 tests/prototypes 评估，设置继续使用 schema 5。当前测试与产物见 CURRENT_STATUS.md，真实输入、性能、签名和物理环境仍需独立证据。
...
8. 在线更新只能读取指定仓库的稳定 Release；凭据使用 Windows 凭据管理器，带凭据请求不得经过第三方加速服务，安装器必须按 Release 清单校验后由用户确认启动。
...
2026-09-12 按 SRCH-08 完成搜索结果选择操作区、安全 Shell 调用、复制路径、快捷入口复用和选择生命周期，并使用 bump-version.ps1 将 0.17.0 升为 MINOR 0.18.0。能力证据、最终测试和产物状态见 [CURRENT_STATUS.md](CURRENT_STATUS.md)。

本次版本号表示用户明确要求的功能候选升级，不表示验收通过或正式发布。历史记录不能替代当次证据；源代码提交/远端同步、真实桌面、物理矩阵、签名及远端资产必须分别验证。未满足完整门禁时，不创建正式标签或 GitHub Release。

当前扩展计划入口为 [0.18.0－功能扩展计划](0.18.0－功能扩展计划.md)，从 0.17.0 源码和测试重新推导；历史计划和 RFC 不代表当前排期。实施结果见 [本次报告](release/0.18.0-test-report.md)。
