...
| 属性 | 值 |
| :--- | :--- |
| 文档版本 | 1.6（2026-09-09 代码状态核对） |
| 软件基线版本 | 0.16.0 |
| 状态 | 生效；WPF/.NET 8 技术栈已落地，0.16.0 仍为候选 |
| 适用范围 | 源码、测试、文档、构建、打包、版本、GitHub 与 AI 工具协作 |
| 需求基线 | `docs/Windows桌面宠物产品需求文档_PRD.md` V2.4 |
...
当前仓库已有 WPF/.NET 8 产品源码、CC0 RGS 桌宠、页内单选与四页工具窗、自动化测试，以及便携版和 Inno Setup 安装版。0.16.0 在 0.15.0 能力上接入 TODAY-01～05 今日安排、已逾期与未安排筛选；EXT-07/08 仍只在 tests/prototypes 评估，产品不引用。当前测试与产物见 CURRENT_STATUS.md，真实输入、性能、签名和物理环境仍需独立证据。
...
2026-09-09 按“好用和必需优先、暂不商业化”的范围实施 TODAY-01～05，并使用 bump-version.ps1 将 0.15.0 升为 MINOR 0.16.0。能力证据、最终测试和产物状态见 [CURRENT_STATUS.md](CURRENT_STATUS.md)。

本次版本号表示用户明确要求的功能候选升级，不表示验收通过或正式发布。历史记录不能替代当次证据；源代码提交/远端同步、真实桌面、物理矩阵、签名及远端资产必须分别验证。未满足完整门禁时，不创建正式标签或 GitHub Release。

当前扩展计划入口为 [0.16.0－功能扩展计划](0.16.0－功能扩展计划.md)，从源码和测试重新推导；历史计划和 RFC 不代表当前排期。实施结果见 [本次报告](release/0.16.0-test-report.md)。
