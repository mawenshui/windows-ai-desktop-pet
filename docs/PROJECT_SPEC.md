...
当前仓库已有 WPF/.NET 8 产品源码、RGS 桌宠资源、自动化测试，以及便携版和 Inno Setup 安装版构建适配器。0.7.1 保持“桌宠主体 + 紧凑工具气泡”，主页搜索结果优先展示，快捷入口支持系统图标、文件夹、编辑、排序、重新定位与拖放添加；搜索输入文字紧贴左侧放大镜图标并保留约半个正文字符间距；工具气泡继续支持按账户持久化的常驻/置顶控制。AI 配置必须由当前值完成连接测试并通过后才能保存，失败时保留输入。0.7.1 本地候选已生成便携包、安装包和校验和；完整便携运行、安装、启动、卸载烟雾验证需在正常交互式 Windows 会话复核，当前受限会话阻止 Windows 凭据管理器和 `{localappdata}` shell folder。完整系统级 E2E、签名、Git 标签和 GitHub Release 仍未完成，GitHub `main` 已完成远端同步。
...
`0.1.0` 的治理初始化仅证明规范、兼容入口和目录骨架存在。`0.7.1` 已完成本地源码、自动化测试、素材许可、便携版、真实交互安装器编译和校验和生成，并已同步 GitHub `main`；完整安装/运行烟雾验证、代码签名、版本标签和 GitHub Release 完成前，只能称为“0.7.1 发布候选”，不能称为正式发布。
