...
| 属性 | 值 |
| :--- | :--- |
| 文档版本 | 2.0（2026-09-12 交互状态修订） |
| 软件基线版本 | 0.18.1 |
| 状态 | 生效；WPF/.NET 8 技术栈已落地，0.18.1 为候选工作区 |
| 适用范围 | 源码、测试、文档、构建、打包、版本、GitHub 与 AI 工具协作 |
| 需求基线 | `docs/Windows桌面宠物产品需求文档_PRD.md` V2.8 |
...
当前仓库已有 WPF/.NET 8 产品源码、CC0 RGS 桌宠、页内单选与四页工具窗、自动化测试，以及便携版和 Inno Setup 安装版。0.18.1 在现有搜索、设置、快捷入口管理和待办编辑流程中统一命令可用状态、已保存基线和反馈，不改变设置 schema 5、待办 schema 4 或权限边界。EXT-08 外部宠物包仍只在 tests/prototypes 评估。当前测试与产物见 CURRENT_STATUS.md，真实输入、性能、签名和物理环境仍需独立证据。
...
2026-09-12 按 UX-181-01～05 完成搜索清空与快捷入口状态、显式保存设置基线、快捷管理上下文动作和待办有效保存条件，并使用 bump-version.ps1 将 0.18.0 升为 PATCH 0.18.1。能力证据、最终测试和产物状态见 [CURRENT_STATUS.md](CURRENT_STATUS.md)。

本次版本号表示用户明确要求的功能候选升级，不表示验收通过或正式发布。历史记录不能替代当次证据；源代码提交/远端同步、真实桌面、物理矩阵、签名及远端资产必须分别验证。未满足完整门禁时，不创建正式标签或 GitHub Release。

当前扩展计划入口为 [0.18.1－功能扩展计划](0.18.1－功能扩展计划.md)，从 0.18.0 源码和测试重新推导；历史计划和 RFC 不代表当前排期。实施结果见 [本次报告](release/0.18.1-test-report.md)。
