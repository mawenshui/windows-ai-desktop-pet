...
当前仓库已有 WPF/.NET 8 产品源码、RGS 桌宠资源、自动化测试，以及便携版和 Inno Setup 安装版构建适配器。0.9.0 在“桌宠主体 + 紧凑工具气泡”基础上增加与普通待办分离的独立提醒项：逐项配置桌宠漫游/跟随气泡，成功投递后自动完成；同时统一修复搜索范围、结果类别、待办筛选、AI 目标与 AI 供应商的下拉选择链路，并包含 0.8.2 的输入渲染等回归修复。所有 AI 创建或变更在用户确认前不写入。最终便携包及隔离安装/启动/凭据/卸载烟雾已通过；真实系统通知、漫游/气泡视觉、休眠、多屏、完整系统级 E2E、签名、Git 标签和 GitHub Release 仍未完成。
...
`0.1.0` 的治理初始化仅证明规范、兼容入口和目录骨架存在。`0.8.0` 完成待办、一次性提醒和确认后写入的 AI 草稿；`0.8.1` 修复三项 UI 回归并固定当前 solution 测试入口；`0.8.2` 修复供应商对象选择和待办输入渲染；`0.9.0` 新增独立提醒项、桌宠跟随气泡/漫游通道，并统一修复所有共享下拉框，当前共 146 项自动化测试通过，最终便携包和隔离安装/卸载烟雾通过。真实通知/漫游/多屏等交互验证、代码签名、版本标签和 GitHub Release 完成前，只能称为“0.9.0 发布候选”，不能称为正式发布。
