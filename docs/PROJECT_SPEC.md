...
当前仓库已有 WPF/.NET 8 产品源码、RGS 桌宠资源、自动化测试，以及便携版和 Inno Setup 安装版构建适配器。0.8.0 保持“桌宠主体 + 紧凑工具气泡”，并把原“计划”占位页升级为本地待办与提醒页：支持手动 CRUD、日期筛选、一次性提醒、页内/托盘提示、启动补发，以及基于已保存 OpenAI 兼容配置的一句话结构化草稿。所有 AI 创建或变更在用户确认前不写入，失败保留输入并可转手动。主页搜索、快捷入口、常驻/置顶和测试后保存 AI 配置的既有行为保持不变。完整便携运行、安装、通知和卸载烟雾验证需在正常交互式 Windows 会话复核；完整系统级 E2E、签名、Git 标签和 GitHub Release 仍未完成。
...
`0.1.0` 的治理初始化仅证明规范、兼容入口和目录骨架存在。`0.8.0` 已完成待办、一次性提醒和确认后写入的 AI 草稿源码及自动化测试；便携版、真实交互安装器和校验和必须在本次打包门禁重新生成。完整安装/运行/通知烟雾验证、代码签名、版本标签和 GitHub Release 完成前，只能称为“0.8.0 发布候选”，不能称为正式发布。
