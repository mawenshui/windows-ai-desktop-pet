...
当前仓库已有 WPF/.NET 8 产品源码、RGS 桌宠资源、自动化测试，以及便携版和 Inno Setup 安装版构建适配器。0.10.0 在“桌宠主体 + 紧凑工具气泡”基础上新增首次搜索授权、流式 staging 索引与失败回滚、范围删除时的暂存数据清理、AI 配置安全删除、未保存修改保护、托盘实时状态/Explorer 恢复和有界安全退出。156 项自动化测试、Windows 凭据往返、便携烟雾、隔离安装、安装后烟雾与卸载已通过，发布资产由唯一 staging 重新生成并核对 SHA-256，并以 `v0.10.0` 正式 GitHub Release 发布。二进制未签名，真实系统通知、Explorer 实际重启、漫游/气泡视觉、休眠、多屏/缩放和独立系统级 E2E 仍作为明确验证债务保留。
