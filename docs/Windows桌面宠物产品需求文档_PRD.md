...
| 文档属性 | 内容 |
| :--- | :--- |
| 文档版本 | V1.1（MVP 草案；补充待办与提醒后续规划） |
| 产品平台 | Windows 桌面端 |
| 文档状态 | 待评审 |
| 需求来源 | 用户本次产品想法；`需求文档.md` 仅作为背景素材参考 |
| 版本范围 | 桌面宠物入口、工具窗口主页、快捷启动、本地搜索、托盘与自启、帮助、AI 接入配置、两个扩展占位页；记录其中一个预留页的“待办与提醒”后续方向 |
...
6. 预留两个与主页同级的导航页面；其中一个页面作为“待办与提醒”的后续候选方向，首版仍只交付中性占位状态，另一个页面继续保留未定义扩展能力。
...
8. 宠物视觉素材、角色设定、动画与声音不是本次输入的确定内容，MVP 仅要求可识别、可点击的宠物入口。
9. 后续可能将“扩展页一”演进为待办与提醒页面，支持手动创建以及通过 AI 对话生成结构化草稿；AI 只负责理解与辅助填写，任何创建、修改或删除均需用户确认后才写入本地数据。
...
- AI 对话、待办与提醒的正式功能、文本/图片生成、AI Agent、自动执行系统操作；“待办与提醒”仅作为后续需求进入本 PRD，不纳入 MVP 发布门槛。
...
6. **配置 AI 服务**：用户在设置中填写必要信息、保存并测试连接；失败时根据分类提示修正配置。
...
| 工具窗口 | 扩展页一 | MVP 为同级导航占位；后续候选演进为“待办与提醒”，包含列表、手动编辑、AI 对话入口和结构化确认。 |
...
- 配置路径：托盘右键 → 设置 → AI 接入 → 填写并保存 → 测试连接 → 查看结果。
- 求助路径：托盘右键 → 帮助 → 打开使用手册 → 查找对应说明。
- 手动待办路径（后续）：点击宠物 → 待办与提醒 → 新建 → 填写标题/时间/提醒 → 确认保存。
- AI 待办路径（后续）：点击宠物 → 待办与提醒 → AI 对话 → 描述事项 → 必要时补充信息 → 检查结构化草稿 → 确认创建。
- 提醒处理路径（后续）：提醒触发 → 查看通知 → 完成/稍后提醒/打开详情 → 同步更新待办状态或下一次提醒时间。
...
| 功能模块 | 功能点 | 优先级 | 详细描述 |
| :--- | :--- | :--- | :--- |
| 桌面宠物 | PET-01 宠物常驻入口 | P0 | 应用运行时显示可点击宠物入口；宠物不因工具窗口收起而退出。视觉素材待定，但点击区域需与可见主体一致。 |
| 桌面宠物 | PET-02 拖动与位置记忆 | P1 | 用户可拖动宠物，位置按 Windows 账户保存；显示器配置变化后自动修正到可视工作区。 |
| 桌面宠物 | PET-03 动画与个性化 | P2 | 提供状态动画、皮肤或更丰富互动；需在独立需求中定义。 |
| 工具窗口 | WIN-01 点击唤出与收起 | P0 | 单击宠物在其上方优先弹出工具窗口；再次点击宠物、点击窗口外或按 Esc 收起，收起不退出应用。发生交互冲突时，窗口内点击不得误触收起。 |
| 工具窗口 | WIN-02 自适应定位 | P0 | 上方空间不足时自动选择下方或侧方，保证主体位于当前显示器工作区；适配多显示器和缩放。 |
...
| 主页搜索 | SRCH-01 模糊搜索输入 | P0 | 主页顶部提供单行输入框，支持按文件/应用名称进行模糊匹配；有效输入后延迟触发，避免每次按键阻塞界面。占位文案说明可搜索文件或应用。 |
| 主页搜索 | SRCH-02 搜索范围管理 | P0 | 设置页可添加、撤销用户授权文件夹，并展示未配置、准备中、可用、失败状态；不默认开启全盘扫描。应用入口来自系统可识别来源，具体来源由技术验证确认。 |
...
| 系统托盘 | TRAY-02 右键菜单 | P0 | 至少包含“显示/隐藏、设置、开机自启、帮助、退出”；菜单项状态与实际状态同步，禁用项需说明原因。 |
...
| AI 接入 | AI-03 保存与连接测试 | P0 | 保存配置不依赖网络；连接测试使用最小无业务数据请求，展示测试中、成功、未验证或分类失败。测试期间防止重复并发提交，可取消或超时结束。 |
...
| 待办与提醒（后续） | TODO-01 页面与列表 | P2 | 将扩展页一演进为待办与提醒页，至少提供“待处理、已完成”状态及按日期查看能力；显示标题、截止/提醒时间、重复状态和完成状态，并具有空状态与新建入口。 |
| 待办与提醒（后续） | TODO-02 手动创建与维护 | P2 | 支持手动创建、查看、编辑、完成、恢复和删除待办；标题必填，备注、截止时间、提醒时间和重复规则可选。删除前需确认，并区分“删除待办”与“仅取消提醒”。 |
| 待办与提醒（后续） | TODO-03 提醒规则 | P2 | 支持一次性提醒；是否支持每日、每周、工作日、自定义重复及多次提醒由后续范围确认。不得保存无效或不可解释的时间规则，时区默认跟随 Windows 当前时区并明确展示绝对日期时间。 |
| 待办与提醒（后续） | TODO-04 通知与用户操作 | P2 | 到期时通过 Windows 通知提醒，并在技术能力允许时提供“完成、稍后提醒、打开详情”；通知权限不可用或投递失败时需可诊断。应用未运行/休眠期间的补发策略须在开发前确认。 |
| 待办与提醒（后续） | TODO-05 AI 对话解析 | P2 | 用户可用自然语言描述事项、日期时间、重复规则和备注；AI 返回结构化草稿。信息缺失、相互冲突或有多种合理解释时，优先追问影响创建结果的最少必要字段，不得静默猜测。 |
| 待办与提醒（后续） | TODO-06 草稿确认与写入 | P2 | 在独立确认卡中展示 AI 解析的标题、截止时间、提醒时间、重复规则、备注及当前时区；用户可修改、确认或取消。仅确认后写入本地，并提供创建成功反馈和撤销入口。 |
| 待办与提醒（后续） | TODO-07 对话式修改 | P2 | 支持通过 AI 发起修改、完成、稍后提醒或删除意图，但必须先明确唯一目标并展示变更前后差异；用户再次确认后才执行，存在多个同名待办时要求用户选择。 |
| 待办与提醒（后续） | TODO-08 AI 降级与隐私 | P2 | 未配置 AI、连接失败、超时、限流或解析失败时保留用户输入并提供重试及“转为手动填写”；只发送本次解析所需内容，不发送其他本地上下文，对话是否保存及保留周期须由用户可见并可清理。 |
...
1. “保存”和“测试连接”是两个独立动作：离线时可以保存，成功保存不等于连接已验证。
2. 修改服务类型、Key、服务地址或模型后，连接状态重置为“未验证”。
3. 测试连接只发送适配器要求的最小请求，不携带搜索词、路径、快捷项、描述或其他本地数据。
4. 错误提示只展示标准化错误类别、时间和建议；原始响应体、鉴权头和 Key 不进入界面、日志或埋点。
5. 连接测试设置明确超时；建议初值为 15 秒，最终由供应商能力与技术验证确认。
...
- [ ] **AC-02（WIN-01、WIN-02）**：给定宠物位于主屏、屏幕上边缘和副屏边缘三种位置，当用户单击宠物，那么 300 毫秒 P95 内出现唯一工具窗口，优先位于上方且主体不超出当前工作区；再次点击宠物、点击窗口外或按 Esc 后窗口收起但应用仍驻留。
- [ ] **AC-03（NAV-01、NAV-02）**：给定工具窗口从隐藏变为显示，当此前停留在任一扩展页，那么本次显示进入主页；当依次切换主页、扩展页一、扩展页二各 20 次，那么页面选中态正确、占位文案可见且无异常或重复实例。
- [ ] **AC-04（SRCH-01）**：给定可用范围含“季度复盘.docx”和应用“复盘助手”，当输入名称片段“复盘”，那么一次延迟查询同时返回两个名称匹配项，输入过程无界面冻结；搜索不读取文档正文。
- [ ] **AC-05（SRCH-02）**：给定用户尚未授权任何文件夹，当进入主页，那么显示添加范围引导且不启动全盘扫描；当添加一个有权限目录后，界面显示准备状态并最终进入可用或可解释的失败状态；撤销后该目录不再出现在搜索结果中。
...
- [ ] **AC-14（TRAY-01、TRAY-02）**：给定应用正常运行，当收起工具窗口时，托盘图标仍存在；右键菜单包含“显示/隐藏、设置、开机自启、帮助、退出”，其中显示状态与实际窗口、自启状态一致。
...
- [ ] **AC-22（AI-03）**：给定设备离线且配置字段合法，当点击保存，那么配置可保存且状态为“未验证”；网络恢复后点击测试，按钮进入进行中且重复点击不产生并发请求，成功后显示“已验证”；测试请求不包含本地查询词、路径或快捷项数据。
...
| F1 待办与提醒需求冻结（后续） | 数据模型、状态机、通知技术方案、AI 对话确认流程、隐私说明 | TODO-01～08 从候选需求转为正式版本范围；提醒运行条件与错过策略可测试 |
| F2 待办与提醒发布候选（后续） | 手动管理、AI 草稿、提醒调度、通知交互、迁移与回归报告 | AC-FUT-01～08 及该版本新增非功能验收全部通过；不影响既有主页与托盘能力 |
