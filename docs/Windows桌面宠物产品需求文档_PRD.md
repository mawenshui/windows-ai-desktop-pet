...
| 文档属性 | 内容 |
| :--- | :--- |
| 文档版本 | V1.7（0.12.1 下拉弹层与自动隐藏可靠性修复） |
| 产品平台 | Windows 桌面端 |
| 文档状态 | 已冻结；0.12.1 下拉选择与工具窗自动隐藏协调实现基线 |
...
| 待办提醒 | 附属于普通待办的一次性通知规则；触发后仍由用户选择完成、稍后提醒或打开详情。 |
| 提醒项 | 与普通待办区分的独立事项；必须包含提醒时间，可逐项启用桌宠漫游、桌宠气泡或两者，成功投递后自动标记为已完成。 |
...
- 待办与独立提醒项列表、手动创建与维护、一次性提醒、桌宠漫游/跟随气泡、运行中页内/托盘提醒和重启后的单次补发。
...
8. **手动管理待办与提醒项**：用户在待办页新增普通待办或独立提醒项；提醒项逐项选择桌宠漫游、气泡或两者，普通待办继续使用完成、编辑、稍后提醒或删除流程。
...
| 工具窗口 | 待办与提醒 | 同级导航页面，包含状态/日期筛选、普通待办与独立提醒项编辑、AI 一句话入口、结构化确认和提醒状态。 |
...
| AI 接入 | AI-01 配置录入 | P0 | 设置中提供服务类型、配置名称和 API Key；支持新增多组命名配置并选择当前使用项；如最终适配器必须使用服务地址、模型等字段，则按所选服务展示并校验。Key 输入默认掩码，可替换和清除。 |
| AI 接入 | AI-02 安全存储 | P0 | Key 使用受当前 Windows 账户保护的安全存储；普通配置只保存非敏感项和凭据引用/状态。日志、埋点、错误提示不得输出 Key。 |
| AI 接入 | AI-03 保存与连接测试 | P0 | 字段完整后允许测试连接；当前供应商、服务地址、模型和 Key 全部测试通过后才允许保存。测试失败、超时或测试期间修改字段时保留输入并禁止保存，不覆盖上一份已保存配置。已保存配置重新测试成功后允许保存验证更新；测试期间防止重复提交并可超时结束。 |
...
| 待办与提醒 | TODO-01 页面与列表 | P1 | 将扩展页一演进为待办与提醒页，至少提供“待处理、已完成”状态及按日期查看能力；显示事项类型、标题、截止/提醒时间、提醒通道和完成状态，并具有空状态与新建入口。 |
| 待办与提醒 | TODO-02 手动创建与维护 | P1 | 支持手动创建、查看、编辑、完成、恢复和删除普通待办，并支持创建、编辑、提前完成和删除提醒项；标题必填。普通待办的备注、截止和提醒可选；提醒项必须设置提醒时间。 |
| 待办与提醒 | TODO-03 提醒规则 | P1 | 0.8.0 支持一次性提醒；每日、每周、工作日、自定义重复及多次提醒留到后续范围。不得保存无效或不可解释的时间规则，时区默认跟随 Windows 当前时区并明确展示绝对日期时间。 |
| 待办与提醒 | TODO-04 通知与用户操作 | P1 | 普通待办提醒到期时保留页内“完成、10 分钟后、打开详情”及托盘提示，触发不自动完成；独立提醒项按逐项配置执行桌宠漫游、跟随气泡或两者，成功投递后自动完成。应用退出时不承诺准点提醒，重启后对未投递记录补发一次。 |
...
| 待办与提醒 | TODO-09 桌宠提醒通道 | P1 | 每个独立提醒项分别保存“桌宠漫游”和“桌宠气泡”开关，可单独、同时或全部关闭；气泡与桌宠保持同一窗口坐标系并在漫游、拖动时跟随。关闭 Windows 动画效果时跳过漫游但保留气泡/托盘反馈。 |
...
7. 任一下拉框展开期间不得因弹层焦点切换自动收起工具窗口；选中项必须同时更新可见文字和业务值。按 Esc 时先关闭已展开下拉框，再次按 Esc 才收起工具窗口。
...
1. “保存”依赖当前配置的连接测试结果：字段未完整或当前值未通过时必须禁用；通过后才可保存。已保存配置可重新测试并保存新的验证时间。
...
6. 测试失败或测试期间修改字段时保留全部输入，不持久化失败配置，也不覆盖上一份已保存配置；成功保存的配置可在下次启动后选择并直接使用。
...
6. 普通待办与独立提醒项采用不同状态机：普通待办的提醒触发不自动完成；独立提醒项只有在通知通道接受投递后才自动完成，失败时保持失败诊断，不伪装为完成。
7. 普通待办的一次性提醒触发后，用户可完成、稍后提醒或打开详情；独立提醒项不显示这组操作条。稍后提醒只更新普通待办的下一次提醒时间，不静默修改截止时间。
8. 独立提醒项必须逐项保存漫游/气泡偏好。气泡在桌宠窗口内呈现并随移动保持视觉关联；系统关闭动画时漫游降级为静态反馈。
9. 系统时区或时间发生变化时，日历型提醒按用户选择的本地时间规则重新计算；倒计时型提醒是否支持及语义需另行确认。任何调整均需在详情中可见。
10. 应用未运行、Windows 休眠或通知被系统禁用时，产品只能在已明确承诺的运行条件内计算送达成功率；恢复后的补发、过期丢弃或汇总策略必须在发布前形成规则并写入手册。
11. 对话请求只携带本次任务所需文本。默认不向 AI 服务发送已有待办列表；仅当用户主动引用待办进行修改且确认所选目标后，才发送完成本次操作所需的最少字段。
...
flowchart TD
    ...
    Z -->|确认| AA[保存待办与提醒规则]
    AA --> AB{事项类型}
    AB -->|普通待办提醒| AC[页内与托盘提示]
    AC -->|完成| AD[更新为已完成]
    AC -->|稍后提醒| AE[仅更新下一次提醒时间]
    AC -->|打开详情| U
    AB -->|独立提醒项| AF[按逐项配置漫游并显示跟随气泡]
    AF --> AG[投递成功后自动完成]
```
...
- [ ] **AC-FUT-02（TODO-05、TODO-06、TODO-09）**：给定用户输入“明天下午 3 点提醒我提交周报”且草稿只有提醒时间、没有截止时间，当 AI 解析成功，那么确认卡展示“创建提醒项”、标题、当前时区、完整年月日及 15:00；确认前零写入，确认后只生成一条默认开启桌宠气泡的提醒项。
...
- [ ] **AC-FUT-06（TODO-03、TODO-04）**：给定普通待办设置了一次性提醒且处于产品承诺的运行条件，当到期时，那么只投递一次；“完成”更新完成状态，“稍后提醒”只更新下一次提醒时间，“打开详情”不自动完成待办。
...
- [ ] **AC-FUT-09（TODO-04、TODO-09）**：给定独立提醒项分别配置“仅漫游、仅气泡、漫游+气泡、两者关闭”，当到期投递时，那么桌宠只执行所选行为，气泡在漫游/拖动中跟随；投递成功后该项自动完成且保留 Delivered 和原定时间，投递失败不自动完成。关闭系统动画时漫游安全降级，其他通道仍可用。
...
| F1 0.9.0 待办与提醒需求冻结 | 数据模型、状态机、通知技术方案、AI 确认流程、隐私说明 | TODO-01～09 已转为正式版本范围；普通待办与独立提醒项边界记录在 `TODO_REMINDER_DESIGN.md` |
