...
| 技术基线 | `docs/TECHNICAL_DESIGN.md` 0.8.1 |
| 软件基线 | `VERSION` 0.8.1 |
| 状态 | 已有可运行 WPF 源码与自动化回归入口；本版本补充 AI 供应商显示、测试后保存状态和快捷入口空状态回归；系统级 E2E、安装/卸载和 Release 验证仍待补齐 |

## 0. 目的

1. 把 PRD 中每条 P0 验收条件（AC-01 ~ AC-24）及 0.8.0 待办验收（AC-FUT-01 ~ AC-FUT-08）映射到可执行测试用例；
...
本版本针对三个已修复的 UI 缺陷增加回归断言；用例必须绑定当前工作树的 `src/AiPet.sln`，不得从 `build/` 或 `dist/` 历史副本运行。
...
| 缺口 | 原因 | 下一阶段动作 |
| :--- | :--- | :--- |
| 集成/E2E 框架与覆盖率门禁未完整落地 | 当前版本的 138 项 xUnit 覆盖待办持久化、手动状态转换、提醒单次投递/补发/失败、AI JSON 与错误分类、确认前零副作用和同名目标选择，并继续覆盖主页快捷项、范围切换、紧凑布局、角色交互、递归名称索引、AI 配置状态机、WPF 控件尺寸、窗口生命周期与 hover 布局稳定；测试可将四页 WPF 实际渲染为 PNG 供视觉检查 | 下一阶段补独立 UI runner、托盘通知区自动化、休眠/时区变化、真实鼠标失焦、多显示器连续拖动帧率与覆盖率报告 |
