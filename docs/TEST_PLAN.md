...
基线：0.15.0，2026-09-08；测试结果以[本次报告](release/0.15.0-test-report.md)为准。实现、进程内 WPF 渲染、独立桌面输入和物理设备验收分别记录。
...
系统当前显示器“存在”和几何合同通过不等于多屏交互已通过。hardware 自动收集只建立 SKIP 清单，需人工操作多屏、100%～200% 缩放、热插拔、休眠、时区/时钟、Explorer 重启、升级保留数据并提供见证记录。0.15.0 还需在解锁的同桌面会话验证两个默认全局组合、改键、占用冲突、停用和退出释放，观察启动自动备份不会重复，并人工走查完整 AI 配置的口令、预览、取消与即用流程。smoke 的静默卸载只覆盖保留分支，交互清理和真实跨版本迁移另测。
