...
| 技术基线 | `docs/TECHNICAL_DESIGN.md` 0.9.0 |
| 软件基线 | `VERSION` 0.9.0 |
| 状态 | 146 项自动化回归、最终便携包自检及隔离安装/启动/凭据/卸载烟雾通过；本版本覆盖独立提醒项、桌宠跟随气泡/漫游、投递后自动完成和全应用下拉选择；真实系统通知、漫游视觉、多屏、系统级 E2E 和远端 Release 验证仍待补齐 |
...
| 缺口 | 原因 | 下一阶段动作 |
| :--- | :--- | :--- |
| 集成/E2E 框架与覆盖率门禁未完整落地 | 当前版本的 146 项 xUnit 覆盖待办持久化、普通/独立提醒状态转换、逐项桌宠通道、投递后自动完成、单次投递/补发/失败、AI JSON 与错误分类、确认前零副作用和同名目标选择，并继续覆盖主页快捷项、全局下拉选择、紧凑布局、角色交互、递归名称索引、AI 配置状态机、WPF 控件尺寸、中文输入渲染、窗口生命周期与 hover 布局稳定；测试可将四页 WPF 实际渲染为 PNG 供视觉检查 | 下一阶段补独立 UI runner、托盘通知区自动化、休眠/时区变化、真实鼠标失焦、多显示器连续拖动帧率与覆盖率报告 |
